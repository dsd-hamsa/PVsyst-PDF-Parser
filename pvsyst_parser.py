...
class PVsystParser:
    ...
    def parse_inverter_range(self, inv_text: str) -> List[str]:
        ...
        for part in parts:
            ...
            m_single = re.search(
                r"(?:[A-Za-z0-9]+_)?INV\s*([A-Za-z]*)(\d+)", part, re.IGNORECASE
            )
            if m_single:
    ...
    def _parse_array_block(self, section_text: str, array_id: str) -> Dict[str, Any]:
        ...
        m_inv_mppt = re.search(
            r"((?:[A-Za-z0-9]+_)?INV\s*[A-Za-z0-9,\-\s]+?)\s+MPPT",
            header_line,
            re.IGNORECASE,
        )
        if m_inv_mppt:
        ...
        if not inverter_ids:
            m_inv_spec = re.search(
                r"((?:[A-Za-z0-9]+_)?INV\s*[A-Za-z0-9,\-\s]+?)(?=\s+MPPT|\s*$)",
                header_line,
                re.IGNORECASE,
            )
            if m_inv_spec:
